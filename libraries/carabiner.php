...
class Carabiner {
    ...
    {
        $carabiner_config = Config::get('carabiner::carabiner');
	...
	*/
	public function config($config)
	{
		...
		if($this->base_uri == '') $this->base_uri = rtrim(Config::get('application.url', ''), '/') . '/';
        ...
        Log::write('debug', 'Carabiner: library configured.');
	}
	...
	*/
	public function group($group_name, $assets)
	{
		...
		if(!isset($assets['js']) && !isset($assets['css']) ){
			Log::write('error', "Carabiner: The asset group definition named '{$group_name}' does not contain a well formed array.");
			return;
		}
	...
	*/
	private function _display_js($group = 'main')
	{
		...
		if( !isset($this->js[$group]) ): // the group you asked for doesn't exist. This should never happen, but better to be safe than sorry.
			...
			Log::write('error', "Carabiner: The JavaScript asset group named '{$group}' does not exist.");
			return;
	...
	*/
	private function _display_css($group = 'main')
	{
		...
		if( !isset($this->css[$group]) ): // the group you asked for doesn't exist. This should never happen, but better to be safe than sorry.
			...
			Log::write('error', "Carabiner: The CSS asset group named '{$group}' does not exist.");
			return;
	...
	*/
	private function _cache($filename, $file_data)
	{
		...
		if(empty($file_data)):
			Log::write('debug', 'Carabiner: Cache file '.$filename.' was empty and therefore not written to disk at '.$this->cache_path);
			return false;
		endif;
		...
		if($success) :
			Log::write('debug', 'Carabiner: Cache file '.$filename.' was written to '.$this->cache_path);
			return TRUE;
		else :
			Log::write('error', 'Carabiner: There was an error writing cache file '.$filename.' to '.$this->cache_path);
			return FALSE;
		endif;
	}
	...
	private function _tag($flag, $ref, $cache = FALSE, $media = 'screen')
	{
		...
		switch($flag){
			...
			case 'js':
				...
				return '<script type="text/javascript" src="'.$dir.$ref.'" charset="'.Config::get('application.encoding').'"></script>'."\r\n";
			...
			case 'css':
				...
				foreach( $files as $file ){
					...
					if ( ($ext == $flag) && $fl >= 42 && ( filemtime( $this->cache_path . $file ) < $before) ) {
						...
						if($success) : Log::write('debug', 'Carabiner: Cache file '.$file.' was removed from '.$this->cache_path);
						else : Log::write('error', 'Carabiner: There was an error removing cache file '.$file.' from '.$this->cache_path);
						endif;
